...
heirloom build -n NAME -i ID -b BASE_PREFIX -r REGION [-d DIRECTORY_TO_BUILD] [-p] [-g] [-e DIRECTORIES_TO_EXCLUDE] [-l LOG_LEVEL]
...
For example: -b 'test' -r 'us-west-1'  will expect bucket 'test-us-west-1' to be present", :type => :string
          opt :directory, "Source directory of build.", :type    => :string, 
                                                        :default => '.'
          opt :exclude, "File(s) or directorie(s) to exclude.", :type  => :string,
                                                                :multi => true
          ...
          opt :region, "Region(s) to upload archive.  Can be set multiple times.", :type  => :string,
                                                                                   :multi => true
